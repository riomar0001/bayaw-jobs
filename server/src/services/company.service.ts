...
export class CompanyService {
  ...
  async updateApplicationStatus(
    ...
    const applicantEmail = application.applicant_profile?.user?.email;
    const applicantFirstName = application.applicant_profile?.user?.first_name ?? '';
    const jobTitle = application.job?.title ?? 'the position';
    const companyInfo = application.job?.company_information;
    const companyContact = companyInfo?.companyContacts?.[0];
    ...
    if (applicantEmail) {
      await emailService.sendApplicationStatusEmail({
        ...
        jobTitle,
        status,
        ...(companyInfo?.company_name && { companyName: companyInfo.company_name }),
        ...(companyInfo?.website && { companyWebsite: companyInfo.website }),
        ...(companyContact?.email && { companyEmail: companyContact.email }),
        ...(companyContact?.phone && { companyPhone: companyContact.phone }),
      });
    }
