...
import { ApplicantStats } from '@/api/types';
...
export default function ApplicantsPage() {
  const [data, setData] = useState<ApplicantStats | null>(null);
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const result = await businessService.getApplicantStats();
      console.log('Fetched applicant stats:', result);
      setData(result);
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : 'Failed to load applicant statistics';
      setError(errorMsg);
      console.error('[useApplicantStats]', err);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="flex flex-1 flex-col gap-6">
        ...
        {!isLoading && !error && data && (
          <>
            {/* Stats */}
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 p-6">
              <StatsCard title="Total Applicants" value={data.total_applicants} icon={Users} />
              <StatsCard title="In Interview" value={data.in_interview} icon={Clock} />
              <StatsCard title="Hired" value={data.hired} icon={UserCheck} />
              <StatsCard title="Rejected" value={data.rejected} icon={XCircle} />
            </div>
            ...
            <div className="rounded-md border bg-card text-card-foreground shadow-sm mx-6 mb-6">
              ...
              <div className="p-6 pt-0">
                <DataTable columns={columns} data={data.applicants ?? []} />
              </div>
            </div>
          </>
        )}
